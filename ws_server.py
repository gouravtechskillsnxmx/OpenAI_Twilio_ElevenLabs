# ws_server_fixed.py
"""
Consolidated, patched ws_server for Twilio Media Streams.
Key fixes included here:
 - Only append "inbound" media frames (ignore "outbound" playback frames)
 - On 'stop' process any remaining inbound buffer before closing
 - Clear and robust TURN processing in a dedicated async helper
 - Audible TTS fallback so you can test without provider keys
 - Explicit logs for every stage (inbound frames, TTS, conversion, outbound frames)
 - Port handling for Render (reads PORT env var when run as script)

Drop this file into your project, set HOSTNAME env var to your public domain,
install ffmpeg in the Docker image (or ensure pydub can decode), and redeploy.
"""
...
PAUSE_MS = int(os.environ.get("PAUSE_MS", "600"))  # silence threshold to detect end of utterance (ms)
...
async def process_turn(ws: WebSocket, stream_sid: str, inbound_ulaw: bytes):
    ...
    try:
        while True:
            ...
            if event == "start":
                ...
                # *** CRITICAL: only consider 'inbound' frames as caller audio.
                # Twilio also sends 'outbound' frames that reflect audio it is playing back (Say/Play).
                # Appending outbound frames will confuse end-of-turn detection.
                if track != "inbound":
                ...
                # process final buffered audio if present
                if len(recorder) > 0 and not processing:
                    ...
                    recorder.clear()
                    await process_turn(ws, stream_sid, inbound)
                    processing = False
                break

            # TURN detection (silence gap)
            now = time.time()
            if (now - last_voice_ts) * 1000.0 > PAUSE_MS and len(recorder) > 0 and not processing:
                processing = True
                inbound = bytes(recorder)
                recorder.clear()
